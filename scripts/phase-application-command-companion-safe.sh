...
LOG_DIR="logs"
mkdir -p "$LOG_DIR"
LOG_FILE="$LOG_DIR/phase-application-command-companion-$(date +%Y%m%d-%H%M%S).log"
...
finish() {
  ...
  if [ "$FAILED" -eq 0 ]; then
    echo "✅ Application Command Companion phase completed successfully." | tee -a "$LOG_FILE"
...
if [ "$FAILED" -eq 0 ]; then
  run "Build checkpoint before Application Companion" npm run build
fi
...
if [ "$FAILED" -eq 0 ]; then
  ...
  if git diff --cached --quiet; then
    ...
    git commit -m "checkpoint before application command companion" 2>&1 | tee -a "$LOG_FILE"
    STATUS=${PIPESTATUS[0]}
...
if [ "$FAILED" -eq 0 ]; then
  ...
  step "Creating Student Application Command Companion"

  cat > src/components/experience/student-application-command-companion.tsx <<'TSX'
"use client";
...
if [ "$FAILED" -eq 0 ]; then
  step "Wiring Student Application Command Companion into MotionProvider"
...
import_line = 'import { StudentApplicationCommandCompanion } from "@/components/experience/student-application-command-companion";'

if import_line not in text:
    if 'import { ContextualExperienceCopilot } from "@/components/experience/contextual-experience-copilot";' in text:
        text = text.replace(
            'import { ContextualExperienceCopilot } from "@/components/experience/contextual-experience-copilot";',
            'import { ContextualExperienceCopilot } from "@/components/experience/contextual-experience-copilot";\n' + import_line
        )
    elif 'import { CinematicJourneyNavigator } from "@/components/experience/cinematic-journey-navigator";' in text:
        text = text.replace(
...
if "<StudentApplicationCommandCompanion />" not in text:
    if "<ContextualExperienceCopilot />" in text:
        text = text.replace(
            "        <ContextualExperienceCopilot />",
            "        <ContextualExperienceCopilot />\n        <StudentApplicationCommandCompanion />"
        )
    elif "<ApplicationJourneyAssistant />" in text:
        text = text.replace(
...
path.write_text(text)
print("✅ Student Application Command Companion wired")
PY
fi

if [ "$FAILED" -eq 0 ]; then
  step "Adding focus mode CSS"

  cat >> src/app/globals.css <<'CSS'

/* Devasiri application focus mode */
:root[data-devasiri-application-focus="true"] input,
:root[data-devasiri-application-focus="true"] textarea,
:root[data-devasiri-application-focus="true"] select {
  box-shadow:
    0 0 0 1px color-mix(in srgb, var(--theme-primary) 35%, transparent),
    0 18px 45px rgba(7, 17, 31, 0.08);
}

:root[data-devasiri-application-focus="true"] input:focus,
:root[data-devasiri-application-focus="true"] textarea:focus,
:root[data-devasiri-application-focus="true"] select:focus {
  box-shadow:
    0 0 0 4px var(--theme-ring),
    0 22px 60px rgba(7, 17, 31, 0.12);
}
CSS

  echo "✅ Focus mode CSS added" | tee -a "$LOG_FILE"
fi
...
if [ "$FAILED" -eq 0 ]; then
  run "Production build after Application Companion" npm run build
fi

if [ "$FAILED" -eq 0 ]; then
  step "Stage Application Companion"

  git add src/components/experience/student-application-command-companion.tsx 2>&1 | tee -a "$LOG_FILE"
  git add src/components/motion/motion-provider.tsx 2>&1 | tee -a "$LOG_FILE"
  git add src/app/globals.css 2>&1 | tee -a "$LOG_FILE"
  echo "✅ Student Application Command Companion staged" | tee -a "$LOG_FILE"
fi

if [ "$FAILED" -eq 0 ]; then
  step "Commit Application Companion"

  if git diff --cached --quiet; then
    echo "ℹ️ No staged implementation changes found" | tee -a "$LOG_FILE"
  else
    git commit -m "add student application command companion" 2>&1 | tee -a "$LOG_FILE"
    STATUS=${PIPESTATUS[0]}

    if [ "$STATUS" -ne 0 ]; then
      echo "❌ Application Companion commit failed safely" | tee -a "$LOG_FILE"
      FAILED=1
    else
      echo "✅ Student Application Command Companion commit created" | tee -a "$LOG_FILE"
    fi
  fi
fi
