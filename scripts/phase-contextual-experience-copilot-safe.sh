...
LOG_DIR="logs"
mkdir -p "$LOG_DIR"
LOG_FILE="$LOG_DIR/phase-contextual-experience-copilot-$(date +%Y%m%d-%H%M%S).log"
...
finish() {
  ...
  if [ "$FAILED" -eq 0 ]; then
    echo "✅ Contextual Experience Copilot phase completed successfully." | tee -a "$LOG_FILE"
...
if [ "$FAILED" -eq 0 ]; then
  run "Build checkpoint before Contextual Copilot" npm run build
fi
...
if [ "$FAILED" -eq 0 ]; then
  ...
  if git diff --cached --quiet; then
    ...
    git commit -m "checkpoint before contextual experience copilot" 2>&1 | tee -a "$LOG_FILE"
    STATUS=${PIPESTATUS[0]}
...
if [ "$FAILED" -eq 0 ]; then
  ...
  step "Creating Contextual Experience Copilot"

  cat > src/components/experience/contextual-experience-copilot.tsx <<'TSX'
"use client";
...
if [ "$FAILED" -eq 0 ]; then
  step "Wiring Contextual Experience Copilot into MotionProvider"
...
import_line = 'import { ContextualExperienceCopilot } from "@/components/experience/contextual-experience-copilot";'

if import_line not in text:
    if 'import { CinematicJourneyNavigator } from "@/components/experience/cinematic-journey-navigator";' in text:
        text = text.replace(
            'import { CinematicJourneyNavigator } from "@/components/experience/cinematic-journey-navigator";',
            'import { CinematicJourneyNavigator } from "@/components/experience/cinematic-journey-navigator";\n' + import_line
        )
    elif 'import { AdaptiveExperienceOrchestrator } from "@/components/experience/adaptive-experience-orchestrator";' in text:
        text = text.replace(
...
if "<ContextualExperienceCopilot />" not in text:
    if "<CinematicJourneyNavigator />" in text:
        text = text.replace(
            "        <CinematicJourneyNavigator />",
            "        <CinematicJourneyNavigator />\n        <ContextualExperienceCopilot />"
        )
    elif "<ExperienceCommandCenter />" in text:
        text = text.replace(
...
path.write_text(text)
print("✅ Contextual Experience Copilot wired")
PY
fi
...
if [ "$FAILED" -eq 0 ]; then
  run "Production build after Contextual Copilot" npm run build
fi

if [ "$FAILED" -eq 0 ]; then
  step "Stage Contextual Experience Copilot"

  git add src/components/experience/contextual-experience-copilot.tsx 2>&1 | tee -a "$LOG_FILE"
  git add src/components/motion/motion-provider.tsx 2>&1 | tee -a "$LOG_FILE"
  echo "✅ Contextual Experience Copilot staged" | tee -a "$LOG_FILE"
fi

if [ "$FAILED" -eq 0 ]; then
  step "Commit Contextual Experience Copilot"

  if git diff --cached --quiet; then
    echo "ℹ️ No staged implementation changes found" | tee -a "$LOG_FILE"
  else
    git commit -m "add contextual experience copilot" 2>&1 | tee -a "$LOG_FILE"
    STATUS=${PIPESTATUS[0]}

    if [ "$STATUS" -ne 0 ]; then
      echo "❌ Contextual Copilot commit failed safely" | tee -a "$LOG_FILE"
      FAILED=1
    else
      echo "✅ Contextual Experience Copilot commit created" | tee -a "$LOG_FILE"
    fi
  fi
fi
