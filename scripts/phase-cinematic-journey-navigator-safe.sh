...
LOG_DIR="logs"
mkdir -p "$LOG_DIR"
LOG_FILE="$LOG_DIR/phase-cinematic-journey-navigator-$(date +%Y%m%d-%H%M%S).log"
...
finish() {
  ...
  if [ "$FAILED" -eq 0 ]; then
    echo "✅ Cinematic Journey Navigator phase completed successfully." | tee -a "$LOG_FILE"
...
for item in package.json src/app src/components src/app/globals.css src/components/motion/motion-provider.tsx; do
  if [ ! -e "$item" ]; then
...
if [ "$FAILED" -eq 0 ]; then
  run "Build checkpoint before Cinematic Journey Navigator" npm run build
fi
...
if [ "$FAILED" -eq 0 ]; then
  ...
  if git diff --cached --quiet; then
    ...
    git commit -m "checkpoint before cinematic journey navigator" 2>&1 | tee -a "$LOG_FILE"
    STATUS=${PIPESTATUS[0]}
...
if [ "$FAILED" -eq 0 ]; then
  ...
  step "Creating Cinematic Journey Navigator"

  cat > src/components/experience/cinematic-journey-navigator.tsx <<'TSX'
"use client";
...
if [ "$FAILED" -eq 0 ]; then
  step "Adding homepage section anchors"

  python3 <<'PY' 2>&1 | tee -a "$LOG_FILE"
from pathlib import Path

replacements = {
    "src/components/ui/hero-video-background.tsx": (
        '<section\n      className={cn(',
        '<section\n      id="home"\n      className={cn('
    ),
    "src/components/blocks/home-experience-showcase.tsx": (
        '<section className="relative overflow-hidden px-6 py-24 sm:px-8 lg:px-12">',
        '<section id="platform-experience" className="relative overflow-hidden px-6 py-24 sm:px-8 lg:px-12">'
    ),
    "src/components/blocks/mission-flow-theater.tsx": (
        '<section className="relative overflow-hidden bg-[var(--theme-surface)] px-6 py-24 sm:px-8 lg:px-12">',
        '<section id="mission-flow" className="relative overflow-hidden bg-[var(--theme-surface)] px-6 py-24 sm:px-8 lg:px-12">'
    ),
    "src/components/blocks/transparency-ledger-theater.tsx": (
        '<section className="relative overflow-hidden bg-[var(--theme-surface)] px-6 py-24 sm:px-8 lg:px-12">',
        '<section id="transparency" className="relative overflow-hidden bg-[var(--theme-surface)] px-6 py-24 sm:px-8 lg:px-12">'
    ),
}
...
for file, (old, new) in replacements.items():
    path = Path(file)
    if not path.exists():
        print(f"ℹ️ Skipped missing file: {file}")
        continue

    text = path.read_text()

    if f'id="{new.split("id=\\"")[1].split("\\"")[0]}"' in text:
        print(f"ℹ️ Anchor already present in {file}")
        continue

    if old in text:
        text = text.replace(old, new, 1)
        path.write_text(text)
        print(f"✅ Added anchor to {file}")
    else:
        print(f"ℹ️ Expected section pattern not found in {file}; skipped safely")
PY
fi

if [ "$FAILED" -eq 0 ]; then
  step "Wiring Cinematic Journey Navigator into MotionProvider"

  python3 <<'PY' 2>&1 | tee -a "$LOG_FILE"
from pathlib import Path

path = Path("src/components/motion/motion-provider.tsx")
text = path.read_text()

import_line = 'import { CinematicJourneyNavigator } from "@/components/experience/cinematic-journey-navigator";'

if import_line not in text:
    if 'import { AdaptiveExperienceOrchestrator } from "@/components/experience/adaptive-experience-orchestrator";' in text:
        text = text.replace(
...
if "<CinematicJourneyNavigator />" not in text:
    if "<AdaptiveExperienceOrchestrator />" in text:
        text = text.replace(
...
path.write_text(text)
print("✅ Cinematic Journey Navigator wired")
PY
fi
...
if [ "$FAILED" -eq 0 ]; then
  run "Production build after Cinematic Journey Navigator" npm run build
fi

if [ "$FAILED" -eq 0 ]; then
  step "Stage Cinematic Journey Navigator"

  git add src/components/experience/cinematic-journey-navigator.tsx 2>&1 | tee -a "$LOG_FILE"
  git add src/components/motion/motion-provider.tsx 2>&1 | tee -a "$LOG_FILE"
  git add src/components/ui/hero-video-background.tsx 2>&1 | tee -a "$LOG_FILE"
  git add src/components/blocks/home-experience-showcase.tsx 2>&1 | tee -a "$LOG_FILE"
  git add src/components/blocks/mission-flow-theater.tsx 2>&1 | tee -a "$LOG_FILE"
  git add src/components/blocks/transparency-ledger-theater.tsx 2>&1 | tee -a "$LOG_FILE"

  echo "✅ Cinematic Journey Navigator staged" | tee -a "$LOG_FILE"
fi

if [ "$FAILED" -eq 0 ]; then
  step "Commit Cinematic Journey Navigator"

  if git diff --cached --quiet; then
    echo "ℹ️ No staged implementation changes found" | tee -a "$LOG_FILE"
  else
    git commit -m "add cinematic journey navigator" 2>&1 | tee -a "$LOG_FILE"
    STATUS=${PIPESTATUS[0]}

    if [ "$STATUS" -ne 0 ]; then
      echo "❌ Cinematic Navigator commit failed safely" | tee -a "$LOG_FILE"
      FAILED=1
    else
      echo "✅ Cinematic Journey Navigator commit created" | tee -a "$LOG_FILE"
    fi
  fi
fi
