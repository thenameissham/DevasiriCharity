...
LOG_DIR="logs"
mkdir -p "$LOG_DIR"
LOG_FILE="$LOG_DIR/phase-volunteer-support-command-$(date +%Y%m%d-%H%M%S).log"
...
finish() {
  ...
  if [ "$FAILED" -eq 0 ]; then
    echo "✅ Volunteer Support Command phase completed successfully." | tee -a "$LOG_FILE"
...
if [ "$FAILED" -eq 0 ]; then
  run "Build checkpoint before new phase" npm run build
fi

if [ "$FAILED" -eq 0 ]; then
  step "Checkpoint any pending successful work"
  ...
  if git diff --cached --quiet; then
    ...
    git commit -m "checkpoint before volunteer support command" 2>&1 | tee -a "$LOG_FILE"
    STATUS=${PIPESTATUS[0]}
...
if [ "$FAILED" -eq 0 ]; then
  ...
  step "Creating Volunteer Support Command Center"

  cat > src/components/blocks/volunteer-support-command.tsx <<'TSX'
"use client";
...
if [ "$FAILED" -eq 0 ]; then
  step "Wiring Volunteer Support Command into homepage"
...
import_line = 'import { VolunteerSupportCommand } from "@/components/blocks/volunteer-support-command";'
...
if "<VolunteerSupportCommand />" not in text:
    if "<TrustStackTheater />" in text:
        text = text.replace(
            "      <TrustStackTheater />",
...
path.write_text(text)
print("✅ Volunteer Support Command wired")
PY
fi
...
if [ "$FAILED" -eq 0 ]; then
  run "Production build after Volunteer Support Command" npm run build
fi

if [ "$FAILED" -eq 0 ]; then
  step "Stage Volunteer Support Command"

  git add src/components/blocks/volunteer-support-command.tsx 2>&1 | tee -a "$LOG_FILE"
  git add src/components/sections/home-page-sections.tsx 2>&1 | tee -a "$LOG_FILE"
  echo "✅ Volunteer Support Command staged" | tee -a "$LOG_FILE"
fi

if [ "$FAILED" -eq 0 ]; then
  step "Commit Volunteer Support Command"

  if git diff --cached --quiet; then
    echo "ℹ️ No staged implementation changes found" | tee -a "$LOG_FILE"
  else
    git commit -m "add volunteer support command center" 2>&1 | tee -a "$LOG_FILE"
    STATUS=${PIPESTATUS[0]}

    if [ "$STATUS" -ne 0 ]; then
      echo "❌ Volunteer Support commit failed safely" | tee -a "$LOG_FILE"
      FAILED=1
    else
      echo "✅ Volunteer Support Command commit created" | tee -a "$LOG_FILE"
    fi
  fi
fi
